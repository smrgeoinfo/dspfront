<template>
  <div class="geodat-submissions">
    <div class="geodat-submissions--header">
      <div class="text-h4">
        My Submissions
      </div>
    ...
    <v-tabs v-model="activeTab" color="primary" class="mt-2 mb-4">
    ...
    <template v-if="activeTab === 'catalog'">
      <template v-if="isFetchingCatalog">
        <v-progress-circular indeterminate color="primary" class="mt-4" />
      </template>
      <template v-else>
        <div v-if="catalogRecords.length" class="mt-4">
          <div class="mb-2 text-h6">
            {{ catalogRecords.length }} Catalog Records
          </div>

          <v-card>
            <div
              v-for="(record, index) in catalogRecords"
              :key="record.id"
            >
              <div class="table-item d-flex justify-space-between flex-column flex-md-row">
                <div class="flex-grow-1 mr-4">
                  ...
                  >
                    <tbody>
                      ...
                      <tr v-if="record.creators">
                        <th class="pr-4 text-body-2">
                          Creators:
                        </th>
                        <td>{{ record.creators }}</td>
                      </tr>
                      <tr>
                        <th class="pr-4 text-body-2">
                          Profile:
                ...
                <div class="d-flex flex-column mt-4 mt-md-0 actions">
                  <v-btn
                    rounded
                    @click="editCatalogRecord(record)"
                  >
                    <v-icon class="mr-1">
                      mdi-pencil-outline
                    </v-icon> Edit
                  ...
                  <v-btn
                    :disabled="catalogDeleting[record.id]"
                    rounded
                    @click="onDeleteCatalogRecord(record)"
                  >
                    <v-icon class="mr-1">
                      mdi-delete-outline
                    </v-icon>
                    {{ catalogDeleting[record.id] ? 'Deleting...' : 'Delete' }}
                  </v-btn>
                </div>
              </div>
              <v-divider />
        ...
        <div v-else class="text-body-2 text-center mt-4">
          <v-empty-state
            title="You have no catalog records yet"
            icon="mdi-text-box-remove"
          />
        </div>
      </template>
    ...
    <v-dialog
      v-model="isCatalogDeleteDialogActive"
      persistent
      width="500"
    >
      <v-card>
        <v-card-title>Delete this catalog record?</v-card-title>
        <v-card-text class="text-body-1">
          <p>This action will permanently delete this metadata record from the catalog.</p>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn
            variant="elevated"
            elevation="1"
            @click="isCatalogDeleteDialogActive = false"
          >
            Cancel
          </v-btn>
          <v-btn
            color="red darken-1"
            variant="elevated"
            @click="confirmDeleteCatalogRecord"
          >
            Delete
          </v-btn>
        </v-card-actions>
...
@Component({
  name: 'geodat-submissions',
  components: { CzRegisterDatasetDialog },
})
class GeodatSubmissions extends mixins(ActiveRepositoryMixin) {
  ...
  async loadCatalogRecords() {
    this.isFetchingCatalog = true
    try {
      this.catalogRecords = await fetchMyRecords()
    }
    catch (e) {
      console.error('Failed to fetch catalog records:', e)
    }
    finally {
  ...
  editCatalogRecord(record: CatalogRecord) {
    const isCdif = record.profile_name === 'CDIFDiscovery'
    if (isCdif) {
      this.router.push({ path: '/metadata/cdif', query: { record: record.id } })
    }
    else {
  ...
  onDeleteCatalogRecord(record: CatalogRecord) {
    this.catalogRecordToDelete = record
    this.isCatalogDeleteDialogActive = true
  }

  async confirmDeleteCatalogRecord() {
    if (!this.catalogRecordToDelete) return
    const record = this.catalogRecordToDelete
    this.isCatalogDeleteDialogActive = false
    this.catalogDeleting[record.id] = true

    try {
      await deleteRecord(record.id)
      this.catalogRecords = this.catalogRecords.filter(r => r.id !== record.id)
      Notifications.toast({ message: 'Record deleted', type: 'success' })
    }
    catch (e) {
      console.error('Failed to delete catalog record:', e)
      Notifications.toast({ message: 'Failed to delete record', type: 'error' })
    }
    finally {
      this.catalogDeleting[record.id] = false
      this.catalogRecordToDelete = null
    }
  }
...
export default toNative(GeodatSubmissions)
</script>
