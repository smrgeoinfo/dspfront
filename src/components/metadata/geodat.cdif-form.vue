<template>
  <v-container class="geodat-cdif-form px-4">
    <h1 class="text-h4">
      Register Dataset with CDIF Metadata
    </h1>
    ...
    >
      <div class="text-black">
        ...
        <p>
          ...
          Press the "Save" button to register your dataset metadata record.
        </p>
      </div>
    </v-alert>
    ...
    <div v-if="!isLoading && schema">
      ...
      <v-alert
        v-if="!isFormValid && errors.length"
        type="warning"
        variant="tonal"
        density="compact"
        class="mt-4 mb-2"
      >
        ...
        <ul class="ml-4">
          <li v-for="(err, i) in errors" :key="i" class="text-body-2">
...
@Component({
  name: 'geodat-cdif-form',
  components: {
    CzForm,
  },
})
class GeodatCdifForm extends Vue {
  route = useRoute()
  router = useRouter()
  ...
  get isFormValid(): boolean {
    if (this.isCategorization) {
      return this.tabValidity.length > 0 && this.tabValidity.every(v => v)
    }
    return this.isValid
  }
  ...
  async loadSchemas() {
    ...
    try {
      const resp = await axios.get(`${CATALOG_API}/profiles/${CDIF_PROFILE}/`)
      ...
      this.errorMessage = 'Failed to load CDIF form schema.'
    }
    finally {
  ...
  goHome() {
    this.router.push({ name: this.isEditMode ? 'submissions' : 'home' })
  }
...
export default toNative(GeodatCdifForm)
</script>
